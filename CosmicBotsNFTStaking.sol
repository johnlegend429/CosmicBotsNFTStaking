...
function addVault(
  // uint vaultId,
        CosmicBotsBPP _nft,
        address _nftContractAdress;
        CosmicBotsCBF _token,
        string calldata _name
    ) public {
        //  require(vaults[vaultId].nftContractAddress == address(0), "Vault already exists");
        VaultInfo.push(
            vaultInfo({
                nft: _nft,
                token: _token,
  ...
  function _claim(address account, uint256[] calldata tokenIds, uint256 _pid, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      tokenId = tokenIds[i];
      Stake memory staked = vault[tokenId];
      Claim memory claimed = vault[tokenId];
      require(staked.owner == account, "not an owner");
      uint256 stakedAt = staked.timestamp;
       uint256 claimedAt = claimed.timestamp;
      require(block.timestamp - claimedAt > 8 hours, "burns nft");
      // vault[tokenId].burnFrom()
